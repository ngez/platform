...
describe("Validators", () => {
  ...
  describe("url()", () => {
    it("should return an error object when invalid", () => {
      ...
      expect(
        NgEzValidators.url(new FormControl("www.google.com"))
      ).toBeTruthy();
      expect(
        NgEzValidators.url(new FormControl("http//www.google.com"))
      ).toBeTruthy();
      expect(
        NgEzValidators.url(new FormControl("http:/www.google.com"))
      ).toBeTruthy();
      ...
      expect(
        NgEzValidators.url(new FormControl("htt://www.google.com"))
      ).toBeTruthy();
      expect(
        NgEzValidators.url(new FormControl("http:///www.google"))
      ).toBeTruthy();
    });

    it("should return null when valid", () => {
      expect(
        NgEzValidators.url(new FormControl("ftp://www.google.com"))
      ).toBeNull();
      expect(
        NgEzValidators.url(new FormControl("http://www.google.com"))
      ).toBeNull();
      expect(
        NgEzValidators.url(new FormControl("https://www.google.com"))
      ).toBeNull();
      expect(
        NgEzValidators.url(
          new FormControl("https://www.google.com/search?q=angular")
        )
      ).toBeNull();
    });
  });
  ...
  describe("fileType()", () => {
    ...
    it("should return an error object if provided a File and an array of errors if provided a FileList or File[]", () => {
      ...
      expect(validator(new FormControl(f3))).toEqual({
        fileType: { accept, actualFile: f3 }
      });
      expect(validator(new FormControl([f3]))).toEqual([
        { fileType: { accept, actualFile: f3 } }
      ]);
      expect(validator(new FormControl([f3, f3]))).toEqual([
        { fileType: { accept, actualFile: f3 } },
        { fileType: { accept, actualFile: f3 } }
      ]);
      expect(validator(new FormControl([f1, f3]))).toEqual([
        { fileType: { accept, actualFile: f3 } }
      ]);
      expect(validator(new FormControl([f1, f2, f3]))).toEqual([
        { fileType: { accept, actualFile: f3 } }
      ]);
    });
  ...
  describe("maxSize()", () => {
    ...
    it("should return an error object if any of the file sizes exceed the limit.", () => {
      const f1 = new File([getRandomText(100000)], "file1.txt", {
        type: "text/plain"
      });
      const f2 = new File([getRandomText(1000001)], "file2.txt", {
        type: "text/plain"
      });
      const f3 = new File([getRandomText(10000000)], "file3.txt", {
        type: "text/plain"
      });

      const validator = NgEzValidators.maxSize(
        NgEzByteUtils.convert(1, "megabyte")
      );
    ...
    it("should return an error object if provided a File and an array of errors if provided a FileList or File[]", () => {
      ...
      expect(validator(new FormControl(f3))).toEqual({
        maxSize: { requiredSize, actualFile: f3 }
      });
      expect(validator(new FormControl([f3]))).toEqual([
        { maxSize: { requiredSize, actualFile: f3 } }
      ]);
      ...
      expect(validator(new FormControl([f1, f2, f3]))).toEqual([
        { maxSize: { requiredSize, actualFile: f2 } },
        { maxSize: { requiredSize, actualFile: f3 } }
      ]);
    });
    ...
    it("should return null if all the file sizes are valid.", () => {
      ...
      const f2 = new File([getRandomText(100000)], "file2.txt", {
        type: "text/plain"
      });
      const f3 = new File([getRandomText(500)], "file3.txt", {
        type: "text/plain"
      });
  ...
  describe("totalSize()", () => {
    ...
    it("should return an error object if the sum of all file sizes exceeds the limit.", () => {
      const f1 = new File([getRandomText(1000)], "file1.txt", {
        type: "text/plain"
      });
      const f2 = new File([getRandomText(1000000)], "file2.txt", {
        type: "text/plain"
      });
      const f3 = new File([getRandomText(1000001)], "file3.txt", {
        type: "text/plain"
      });

      const validator = NgEzValidators.totalSize(
        NgEzByteUtils.convert(1, "megabyte")
      );
    ...
    it("should return null if the sum of all file sizes is valid.", () => {
      const f1 = new File([getRandomText(1)], "file1.txt", {
        type: "text/plain"
      });
      const f2 = new File([getRandomText(1000000)], "file2.txt", {
        type: "text/plain"
